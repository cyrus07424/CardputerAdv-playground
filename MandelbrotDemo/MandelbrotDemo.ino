...
inline int16_t fractal_width() { return display_width(); }
inline int16_t fractal_height() { return display_height() - HEADER_H - FOOTER_H; }
#define FRACTAL_W fractal_width()
#define FRACTAL_H fractal_height()
...
uint16_t* g_fractal_pixels = nullptr;
uint16_t* g_prev_fractal_pixels = nullptr;
...
size_t fractal_pixel_count() {
  return static_cast<size_t>(app_config::FRACTAL_W) * static_cast<size_t>(app_config::FRACTAL_H);
}

size_t fractal_pixel_bytes() {
  return fractal_pixel_count() * sizeof(uint16_t);
}

uint16_t& fractal_pixel(uint16_t* pixels, int16_t x, int16_t y) {
  return pixels[static_cast<size_t>(y) * static_cast<size_t>(app_config::FRACTAL_W) + static_cast<size_t>(x)];
}
...
bool init_fractal_buffers() {
  g_fractal_pixels = static_cast<uint16_t*>(calloc(fractal_pixel_count(), sizeof(uint16_t)));
  g_prev_fractal_pixels = static_cast<uint16_t*>(calloc(fractal_pixel_count(), sizeof(uint16_t)));
  if (g_fractal_pixels != nullptr && g_prev_fractal_pixels != nullptr) {
    return true;
  }
  free(g_fractal_pixels);
  free(g_prev_fractal_pixels);
  g_fractal_pixels = nullptr;
  g_prev_fractal_pixels = nullptr;
  return false;
}
    ...
    for (int16_t x = 0; x < app_config::FRACTAL_W; ++x) {
      ...
      if (src_y >= 0 && src_y < app_config::FRACTAL_H) {
        ...
        if (src_x >= 0 && src_x < app_config::FRACTAL_W) {
          color = fractal_pixel(g_prev_fractal_pixels, src_x, src_y);
        }
      }

      fractal_pixel(g_fractal_pixels, x, y) = color;
...
void invalidate_render(bool reuse_previous = false,
  ...
  g_render_row = 0;
  if (reuse_previous) {
    memcpy(g_prev_fractal_pixels, g_fractal_pixels, fractal_pixel_bytes());
    reuse_previous_pixels(old_center_x, old_center_y, old_view_width);
  } else {
    memset(g_fractal_pixels, 0, fractal_pixel_bytes());
    g_canvas.fillRect(
...
void render_rows(uint8_t row_budget) {
  ...
  while (rows_rendered < row_budget && g_render_row < app_config::FRACTAL_H) {
    ...
    for (int16_t x = 0; x < app_config::FRACTAL_W; ++x) {
      ...
      fractal_pixel(g_fractal_pixels, x, static_cast<int16_t>(g_render_row)) = color;
...
void setup() {
  ...
  if (!init_fractal_buffers()) {
    show_fatal_error("Fractal buffer alloc failed");
  }
