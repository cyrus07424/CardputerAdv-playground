...
inline int16_t render_width() { return display_width(); }
inline int16_t render_height() { return display_height(); }
#define SCREEN_W display_width()
#define SCREEN_H display_height()
#define RENDER_W render_width()
#define RENDER_H render_height()
...
float* g_column_factor = nullptr;
float* g_row_factor = nullptr;
...
uint16_t* g_sky_colors = nullptr;
uint16_t* g_ground_colors = nullptr;
uint16_t* g_scanline = nullptr;
...
void free_render_buffers() {
  free(g_column_factor);
  free(g_row_factor);
  free(g_sky_colors);
  free(g_ground_colors);
  free(g_scanline);
  g_column_factor = nullptr;
  g_row_factor = nullptr;
  g_sky_colors = nullptr;
  g_ground_colors = nullptr;
  g_scanline = nullptr;
}

bool init_render_buffers() {
  free_render_buffers();
  g_column_factor = static_cast<float*>(calloc(app_config::RENDER_W, sizeof(float)));
  g_row_factor = static_cast<float*>(calloc(app_config::RENDER_H, sizeof(float)));
  g_sky_colors = static_cast<uint16_t*>(calloc(app_config::RENDER_H, sizeof(uint16_t)));
  g_ground_colors = static_cast<uint16_t*>(calloc(app_config::RENDER_H, sizeof(uint16_t)));
  g_scanline = static_cast<uint16_t*>(calloc(app_config::RENDER_W, sizeof(uint16_t)));
  if (g_column_factor != nullptr &&
      g_row_factor != nullptr &&
      g_sky_colors != nullptr &&
      g_ground_colors != nullptr &&
      g_scanline != nullptr) {
...
void setup() {
  ...
  if (!init_render_buffers()) {
    show_fatal_error("Render buffer alloc failed");
  }
